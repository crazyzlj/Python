...
def WriteLineShp(lineList, outShp, lineNameList = None):
    ...
    fieldName = []
    fieldNameIdx = 0
    if lineNameList is not None:
        if len(lineList) != len(lineNameList):
            if len(lineList) + 1 == len(lineNameList):
                fieldName = lineNameList[0]
                fieldNameIdx = 1
            else:
                sys.exit(1)
        else:
            fieldLength = len(lineNameList[0])
    ...
    for l in lineList:
        ...
        if len(l) > 1:
            ...
            for fld in fieldName:
                ...
                if lineNameList is not None:
                    if fieldNameIdx == 1:
                        fieldValue = lineNameList[idx+1][idx2]
                    else:
                        fieldValue = lineNameList[idx][idx2]
                else:
                    fieldValue = ' '
                #print fieldValue
